...
**Observed data point (2026-07-31):** the estimate above is a planning assumption, not measured spend. A real end-to-end `digest run` was executed against live services — one broad topic ("AI agents," no tracked handles), 40 posts fetched, 37 themes summarized and drafted — and cost **~$0.41 total** (~$0.0016 data reads + ~$0.406 LLM, almost entirely Summarize/Draft Post calls across the 37 themes). This is real spend from an actual run, not a projection, but it isn't directly comparable in scale to the ~$13–16/month baseline above (that assumes 5 topics × 200 posts/topic/run × 30 runs/month; this was 1 topic, 1 run, on a topic broad enough with no tracked handles to fragment into an unusually high theme count). The baseline estimate remains the planning assumption this real sample is weighed against, not a replacement for it.
